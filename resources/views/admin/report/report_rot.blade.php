...
if (count($round) > 0){
    ...
    //attivato test?
    $p_ref_sub=$fat_ref_sub=$lac_ref_sub=$u_ref_sub=$bhb_sub=$pag_sub="No";

    //question 1 e 2
    $p_ref_sample=$fat_ref_sample=$lac_ref_sample=$u_ref_sample=$bhb_sample=$pag_sample="No";
    $p_ref_q2=$fat_ref_q2=$lac_ref_q2=$u_ref_q2=$scc_ref_q2=$bhb_q2=$pag_q2="";

    //tutti non spuntati
    $p_a=$f_a=$l_a=$u_a=$s_a=$b_a=$pag_a=$bhb_a=0;

    $protein_ref_labcode=$fat_ref_labcode=$lactose_ref_labcode=$urea_ref_labcode=$bhb_labcode=$pag_labcode="&nbsp;";
    $protein_ref_x100=$fat_ref_x100=$lactose_ref_x100=$urea_ref_x100=$bhb_x100=$pag_x100="&nbsp;";
    $protein_ref_dev=$fat_ref_dev=$lactose_ref_dev=$urea_ref_dev=$bhb_dev=$pag_dev="&nbsp;";
    $protein_ref_sdev=$fat_ref_sdev=$lactose_ref_sdev=$urea_ref_sdev=$bhb_sdev=$pag_sdev="&nbsp;";
    $protein_ref_dist=$fat_ref_dist=$lactose_ref_dist=$urea_ref_dist=$bhb_dist=$pag_dist="&nbsp;";
    $protein_ref_m=$fat_ref_m=$lactose_ref_m=$urea_ref_m=$bhb_m=$pag_m="&nbsp;";
    ...
    foreach ($round as $r){
        ...
        switch ($r->code_test) {
            ...
            case 'bhb':
                $bhb_sub     = "Yes";
                $bhb_sample  = ($r->question1)?"Yes":"No";
                $bhb_q2      = ($r->question2)?"Yes":"No";
                $bhb_a=1;
                break;
            case 'pag':
                $pag_sub     = "Yes";
    ...
    foreach ($data as $d){
        switch ($d->type) {
            ...
            case 'urea_ref':
                ...
                break;
            case 'bhb':
                $bhb_labcode = $d->lab_code;
                $bhb_x100    = $d->percent;
                $bhb_dev     = number_format($d->dev,3);
                $bhb_sdev    = number_format($d->s_dev,3);
                $bhb_dist    = number_format($d->dist,3);
                $bhb_m       = $d->method;
                break;
            case 'pag':
                $pag_labcode = $d->lab_code;
                $pag_x100    = $d->percent;
                $pag_dev     = number_format($d->dev,3);
                $pag_sdev    = number_format($d->s_dev,3);
                $pag_dist    = number_format($d->dist,3);
                $pag_m       = $d->method;
                break;
        }
    }
...
@section('content')
    {!! Form::open(array(
               ...
               'route' => ['round_labs'])) !!}
    {{ csrf_field() }}
    <input name="round_id" type="hidden" value={{$code_round}}>
        ...
        <div class="panel-body table-responsive">
            ...
            <head>
                ...
                <style>
                    ...
                    #header .title2 { width: 575px; color: #06C; font-size: 18px; font-weight:bold; border: none;
            ...
            <table id="header">
                <tr>
                    ...
                    <td class="title2">Routine Methods<br />Laboratory participation codes and Performance analyses</td>
            ...
            <table cellspacing="0" id="a">
                ...
                <tr>
                    ...
                    <td class=@if ($bhb_sub=='Yes') {{$class}} @endif>{{$bhb_sub}}</td>
                ...
                <tr>
                    ...
                    <td class=@if ($bhb_sample=='Yes') {{$class}} @endif>{{$bhb_sample}}</td>
            ...
            <table cellspacing="0" id="c">
                ...
                <tr>
                    ...
                    <td class=@if ($bhb_q2=='Yes') {{$class}}  @elseif ($bhb_q2=='No'){{$class_red}} @else
                        {{$class_no}}@endif>{{$bhb_q2}}</td>
            ...
            <table cellspacing="0" id="d">
                ...
                <tr>
                    ...
                    <!-- range: tra -10% e +10%  -->
                    <td class=
                    @if ($bhb_dev == '&nbsp;') {{''}} @elseif (('-10' <= $bhb_dev) && ($bhb_dev <= '10')) {{$class}}
                            @else {{$class_red}} @endif>{{$bhb_dev}}
                    </td>
                ...
                <tr>
                    ...
                    <!-- limite: 10 -->
                    <td class=
                    @if ($bhb_sdev == '10' || $bhb_sdev == '&nbsp;') {{''}} @elseif ($bhb_sdev >'10')  {{$class_red}}
                            @elseif($bhb_sdev < '10'){{$class}} @endif>{{$bhb_sdev}}</td>

                    <!-- limite: 0,045 -->
                    <td class=
                    @if ($pag_sdev == '0,045' || $pag_sdev == '&nbsp;') {{''}} @elseif ($pag_sdev >'0,045') {{$class_red}}
                            @elseif($pag_sdev < '0,045'){{$class}} @endif>{{$pag_sdev}}</td>
                </tr>
                <tr>
            ...
            <table cellspacing="0" id="e">
                ...
                @for($v=0; $v<10; $v++)
                    <tr>
                        ...
                        echo "<td  class=".$class_bhb.">".$outlier[$v]->bhb."</td>";
            ...
            <table cellspacing="0" id="f">
                ...
                @for($v=0; $v<10; $v++)
                    <tr>
                        ...
                        if ($arr_sp1[$v]->fat_rout>'0.043')  $class1="red";
                        elseif ($arr_sp1[$v]->fat_rout=="") $class1="";
                        elseif($arr_sp1[$v]->fat_rout<'0.043') $class1="green"; elseif($arr_sp1[$v]->fat_rout=='0.043') $class1="";


                        if ($arr_sp1[$v]->protein_rout>'0.038')  $class2="red";
                        elseif ($arr_sp1[$v]->protein_rout=="") $class2="";
                        elseif($arr_sp1[$v]->protein_rout<'0.038') $class2="green"; elseif($arr_sp1[$v]->protein_rout=='0.038') $class2="";

                        if ($arr_sp1[$v]->lactose_rout>'0.06')  $class3="red";
                        elseif ($arr_sp1[$v]->lactose_rout=="") $class3="";
                        elseif($arr_sp1[$v]->lactose_rout<'0.06') $class3="green";  elseif
                        ($arr_sp1[$v]->lactose_rout=='0.06') $class3="";

                        if ($arr_sp1[$v]->urea_rout>'1.52')  $class4="red";
                        elseif ($arr_sp1[$v]->urea_rout=="") $class4="";
                        elseif($arr_sp1[$v]->urea_rout<'1.52') $class4="green";  elseif($arr_sp1[$v]->urea_rout=='1.52') $class4="";

                        if ($arr_sp1[$v]->pag>'0.03')  $class6="red";
                        elseif ($arr_sp1[$v]->pag=="") $class6="";
                        elseif($arr_sp1[$v]->pag<'0.03') $class6="green"; elseif($arr_sp1[$v]->pag=='0.03') $class6="";
