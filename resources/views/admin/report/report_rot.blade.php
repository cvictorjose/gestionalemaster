    ...
    foreach ($data as $d){
        switch ($d->type) {
            case 'protein_rout':
                ...
                $protein_ref_dev     = number_format($d->dev,5);
                $protein_ref_sdev    = number_format($d->s_dev,5);
                $protein_ref_dist    = number_format($d->dist,5);
                $protein_ref_m       = $d->method;
                break;
            case 'fat_rout':
                $fat_ref_labcode = $d->lab_code;
                $fat_ref_x100    = $d->percent;
                $fat_ref_dev     = number_format($d->dev,5);
                $fat_ref_sdev    = number_format($d->s_dev,5);
                $fat_ref_dist    = number_format($d->dist,5);
                $fat_ref_m       = $d->method;
                break;
            case 'lactose_rout':
                $lactose_ref_labcode = $d->lab_code;
                $lactose_ref_x100    = $d->percent;
                $lactose_ref_dev     = number_format($d->dev,5);
                $lactose_ref_sdev    = number_format($d->s_dev,5);
                $lactose_ref_dist    = number_format($d->dist,5);
                $lactose_ref_m       = $d->method;
                break;
            case 'urea_rout':
                $urea_ref_labcode = $d->lab_code;
                $urea_ref_x100    = $d->percent;
                $urea_ref_dev     = number_format($d->dev,5);
                $urea_ref_sdev    = number_format($d->s_dev,5);
                $urea_ref_dist    = number_format($d->dist,5);;
                $urea_ref_m       = $d->method;
                break;
            case 'bhb':
                $bhb_labcode = $d->lab_code;
                $bhb_x100    = $d->percent;
                $bhb_dev     = number_format($d->dev,5);
                $bhb_sdev    = number_format($d->s_dev,5);
                $bhb_dist    = number_format($d->dist,5);;
                $bhb_m       = $d->method;
                break;
            case 'pag':
                $pag_labcode = $d->lab_code;
                $pag_x100    = $d->percent;
                $pag_dev     = number_format($d->dev,5);
                $pag_sdev    = number_format($d->s_dev,5);
                $pag_dist    = number_format($d->dist,5);;
                $pag_m       = $d->method;
                break;
        }
    }
